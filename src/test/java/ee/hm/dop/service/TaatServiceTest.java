package ee.hm.dop.service;

import static ee.hm.dop.utils.ConfigurationProperties.KEYSTORE_FILENAME;
import static ee.hm.dop.utils.ConfigurationProperties.KEYSTORE_PASSWORD;
import static ee.hm.dop.utils.ConfigurationProperties.KEYSTORE_SIGNING_ENTITY_ID;
import static ee.hm.dop.utils.ConfigurationProperties.KEYSTORE_SIGNING_ENTITY_PASSWORD;
import static ee.hm.dop.utils.ConfigurationProperties.TAAT_ASSERTION_CONSUMER_SERVICE_INDEX;
import static ee.hm.dop.utils.ConfigurationProperties.TAAT_CONNECTION_ID;
import static ee.hm.dop.utils.ConfigurationProperties.TAAT_SSO;
import static org.easymock.EasyMock.anyObject;
import static org.easymock.EasyMock.createMock;
import static org.easymock.EasyMock.createMockBuilder;
import static org.easymock.EasyMock.expect;
import static org.easymock.EasyMock.expectLastCall;
import static org.easymock.EasyMock.replay;
import static org.easymock.EasyMock.verify;
import static org.junit.Assert.assertEquals;
import static org.junit.Assert.assertFalse;
import static org.junit.Assert.assertNotNull;
import static org.junit.Assert.assertTrue;
import static org.junit.Assert.fail;

import java.io.IOException;
import java.lang.reflect.Method;
import java.util.List;

import javax.servlet.http.HttpServletResponse;
import javax.xml.parsers.ParserConfigurationException;

import org.apache.commons.configuration.Configuration;
import org.apache.xml.security.exceptions.Base64DecodingException;
import org.easymock.EasyMockRunner;
import org.easymock.Mock;
import org.easymock.TestSubject;
import org.joda.time.DateTime;
import org.joda.time.Interval;
import org.junit.Test;
import org.junit.runner.RunWith;
import org.opensaml.common.SAMLObject;
import org.opensaml.common.SAMLVersion;
import org.opensaml.common.binding.BasicSAMLMessageContext;
import org.opensaml.saml2.core.Attribute;
import org.opensaml.saml2.core.AuthnRequest;
import org.opensaml.saml2.core.Issuer;
import org.opensaml.saml2.core.NameIDPolicy;
import org.opensaml.saml2.core.Response;
import org.opensaml.saml2.core.impl.AuthnRequestBuilder;
import org.opensaml.xml.io.UnmarshallingException;
import org.opensaml.xml.security.credential.Credential;
import org.opensaml.xml.security.x509.X509Credential;
import org.opensaml.xml.signature.Signature;
import org.opensaml.xml.signature.SignatureValidator;
import org.opensaml.xml.validation.ValidationException;
import org.xml.sax.SAXException;

import ee.hm.dop.model.AuthenticatedUser;
import ee.hm.dop.model.AuthenticationState;
import ee.hm.dop.model.User;

@RunWith(EasyMockRunner.class)
public class TaatServiceTest {

    @Mock
    private Configuration configuration;

    @Mock
    private AuthenticationStateService authenticationStateService;

    @Mock
    private LoginService loginService;

    @Mock
    private Credential credential;

    @Mock
    private AuthenticationState authenticationState;

    @Mock
    private Response response;

    @Mock
    private Signature signature;

    @Mock
    private SignatureValidator validator;

    @Mock
    private UserService userService;

    @Mock
    private List<Attribute> attributes;

    @Mock
    private AuthenticatedUser authenticatedUser;

    @Mock
    private User user;

    @TestSubject
    private TaatService taatService = new TaatServicePartialMock();

    @TestSubject
    private TaatService taatServiceWithMockedMethods = getTaatServiceWithMockedMethods();

    @Test
    public void buildAuthnRequest() {
        Integer assertionConsumerServiceIndex = 3;
        String connectionId = "https://app.website/sp";

        expect(configuration.getString(TAAT_ASSERTION_CONSUMER_SERVICE_INDEX))
                .andReturn(String.valueOf(assertionConsumerServiceIndex));
        expect(configuration.getString(TAAT_CONNECTION_ID)).andReturn(connectionId);

        replayAll();
        AuthnRequest authnRequest = taatService.buildAuthnRequest();

        verifyAll();
        assertNotNull(authnRequest);
        assertNotNull(authnRequest.getID());
        Interval interval = new Interval(authnRequest.getIssueInstant(), new DateTime());
        assertFalse(authnRequest.getIssueInstant().isAfterNow());
        assertTrue(interval.toDurationMillis() < 1000);
        assertEquals(assertionConsumerServiceIndex, authnRequest.getAssertionConsumerServiceIndex());
        assertNotNull(authnRequest.getIssuer());
        assertEquals(connectionId, authnRequest.getIssuer().getValue());
        assertEquals(SAMLVersion.VERSION_20, authnRequest.getVersion());
        NameIDPolicy nameIdPolicy = authnRequest.getNameIDPolicy();
        assertNotNull(nameIdPolicy);
        assertEquals("urn:oasis:names:tc:SAML:2.0:nameid-format:transient", nameIdPolicy.getFormat());
        assertTrue(nameIdPolicy.getAllowCreate());
    }

    @Test
    public void buildMessageContext() {

        AuthnRequestBuilder authRequestBuilder = new AuthnRequestBuilder();
        AuthnRequest authnRequest = authRequestBuilder
                .buildObject("urn:oasis:names:tc:SAML:2.0:protocol", "AuthnRequest", "samlp");

        HttpServletResponse response = createMock(HttpServletResponse.class);
        AuthenticationState authenticationState = createMock(AuthenticationState.class);

        String endpointLocation = "https://test.taat.ee/notrealurl";

        expect(configuration.getString(KEYSTORE_FILENAME)).andReturn("test.keystore");
        expect(configuration.getString(KEYSTORE_PASSWORD)).andReturn("newKeyStorePass");
        expect(configuration.getString(TAAT_SSO)).andReturn("https://test.taat.ee/notrealurl");
        expect(configuration.getString(KEYSTORE_SIGNING_ENTITY_ID)).andReturn("testAlias");
        expect(configuration.getString(KEYSTORE_SIGNING_ENTITY_PASSWORD)).andReturn("newKeyPass");
        expect(authenticationStateService.create(anyObject(AuthenticationState.class))).andReturn(authenticationState);

        replayAll();
        BasicSAMLMessageContext<SAMLObject, AuthnRequest, SAMLObject> context = taatService
                .buildMessageContext(authnRequest, response);

        verifyAll();
        assertEquals(endpointLocation, context.getPeerEntityEndpoint().getLocation());
        assertEquals(authnRequest, context.getOutboundSAMLMessage());
        assertNotNull(context.getRelayState());
    }

    @Test
    public void getIssuer() {
        String connectionId = "test.taat.edu";

        Issuer issuer = taatService.getIssuer(connectionId);

        assertEquals(connectionId, issuer.getValue());
    }

    @Test
    public void authenticateNullAuthenticationState() throws Exception {
        String authenticationStateToken = "token";

        expect(authenticationStateService.getAuthenticationStateByToken(authenticationStateToken)).andReturn(null);

        replayAll();
        try {
            taatService.authenticate(getSAMLResponse(), authenticationStateToken);
            fail("Exception expected.");
        } catch (RuntimeException e) {
            //correct
        }

        verifyAll();
    }

    @Test
    public void authenticateResponseException() throws Exception {
        String authenticationStateToken = setAuthenticationExpects();
        expect(taatServiceWithMockedMethods.getResponse(getSAMLResponse())).andThrow(new RuntimeException());

        replayAll();

        try {
            taatServiceWithMockedMethods.authenticate(getSAMLResponse(), authenticationStateToken);
            fail("Exception expected.");
        } catch (RuntimeException e) {
            //correct
        }

        verifyAll();
    }

    @Test
    public void authenticateErrorValidatingSignature() throws Exception {
        String authenticationStateToken = setAuthenticationExpects();
        setValidationExpects();

        validator.validate(signature);
        expectLastCall().andThrow(new RuntimeException());

        replayAll();

        try {
            taatServiceWithMockedMethods.authenticate(getSAMLResponse(), authenticationStateToken);
            fail("Exception expected.");
        } catch (RuntimeException e) {
            //correct
        }

        verifyAll();
    }

    @Test
    public void authenticateFirstTimeLogin() throws Exception {
        String authenticationStateToken = setAuthenticationExpects();
        setValidationExpects();
        setDataParsingExpects();

        expect(userService.getUserByIdCode("idCode")).andReturn(null);
        authenticatedUser.setFirstLogin(true);
        expectLastCall();
        expect(userService.create(user)).andReturn(user);
        authenticatedUser.setUser(user);
        expectLastCall();
        expect(loginService.createAuthenticatedUser(authenticatedUser)).andReturn(authenticatedUser);

        replayAll();

        AuthenticatedUser returnedAuthenticatedUser = taatServiceWithMockedMethods
                .authenticate(getSAMLResponse(), authenticationStateToken);

        verifyAll();

        assertNotNull(returnedAuthenticatedUser);
    }

    @Test
    public void authenticateOldUser() throws Exception {
        String authenticationStateToken = setAuthenticationExpects();
        setValidationExpects();
        setDataParsingExpects();
        expect(userService.getUserByIdCode("idCode")).andReturn(user);
        authenticatedUser.setUser(user);
        expectLastCall();
        expect(loginService.createAuthenticatedUser(authenticatedUser)).andReturn(authenticatedUser);

        replayAll();

        AuthenticatedUser returnedAuthenticatedUser = taatServiceWithMockedMethods
                .authenticate(getSAMLResponse(), authenticationStateToken);

        verifyAll();

        assertNotNull(returnedAuthenticatedUser);
    }

    private void setDataParsingExpects() throws ValidationException {
        validator.validate(signature);
        expectLastCall();
        expect(taatServiceWithMockedMethods.getAttributes(response)).andReturn(attributes);
        expect(taatServiceWithMockedMethods.getUserData(attributes)).andReturn(authenticatedUser);
        expect(authenticatedUser.getUser()).andReturn(user);

        expect(taatServiceWithMockedMethods.newUserService()).andReturn(userService);
        expect(user.getIdCode()).andReturn("idCode");
    }

    private class TaatServicePartialMock extends TaatService {
        @Override
        protected AuthenticationStateService newAuthenticationStateService() {
            return authenticationStateService;
        }

        @Override
        protected LoginService newLoginService() {
            return loginService;
        }

        @Override
        protected X509Credential getCredential() {
            return (X509Credential) credential;
        }
    }

    private TaatService getTaatServiceWithMockedMethods() {
        Method getResponse = null;
        Method newAuthenticationStateService = null;
        Method newLoginService = null;
        Method getCredential = null;
        Method newUserService = null;
        Method getSignatureValidator = null;
        Method getAttributes = null;
        Method getUserData = null;

        try {
            getResponse = TaatService.class.getDeclaredMethod("getResponse", String.class);
            newAuthenticationStateService = TaatService.class.getDeclaredMethod("newAuthenticationStateService");
            newLoginService = TaatService.class.getDeclaredMethod("newLoginService");
            getCredential = TaatService.class.getDeclaredMethod("getCredential");
            newUserService = TaatService.class.getDeclaredMethod("newUserService");
            getSignatureValidator = TaatService.class.getDeclaredMethod("getSignatureValidator", Credential.class);
            getAttributes = TaatService.class.getDeclaredMethod("getAttributes", Response.class);
            getUserData = TaatService.class.getDeclaredMethod("getUserData", List.class);
        } catch (NoSuchMethodException e) {
            e.printStackTrace();
        }

        return createMockBuilder(TaatService.class)
                .addMockedMethods(getResponse, newAuthenticationStateService, newLoginService, getCredential,
                        newUserService, getSignatureValidator, getAttributes, getUserData).createMock();
    }

    private String getSAMLResponse() {
        return "";
    }

    private void replayAll() {
        replay(configuration, authenticationStateService, credential, loginService, authenticationState,
                taatServiceWithMockedMethods, response, signature, userService, validator, attributes,
                authenticatedUser, user);
    }

    private void verifyAll() {
        verify(configuration, authenticationStateService, credential, loginService, authenticationState,
                taatServiceWithMockedMethods, response, signature, userService, validator, attributes,
                authenticatedUser, user);
    }

    private String setAuthenticationExpects() {
        String authenticationStateToken = "token";

        expect(taatServiceWithMockedMethods.newLoginService()).andReturn(loginService);
        expect(taatServiceWithMockedMethods.newAuthenticationStateService()).andReturn(authenticationStateService);
        expect(authenticationStateService.getAuthenticationStateByToken(authenticationStateToken))
                .andReturn(authenticationState);
        authenticationStateService.delete(authenticationState);
        expectLastCall();
        return authenticationStateToken;
    }

    private void setValidationExpects()
            throws Base64DecodingException, ParserConfigurationException, SAXException, IOException,
            UnmarshallingException {
        expect(taatServiceWithMockedMethods.getResponse(getSAMLResponse())).andReturn(response);
        expect(response.getSignature()).andReturn(signature);
        expect(taatServiceWithMockedMethods.getSignatureValidator(credential)).andReturn(validator);
    }
}
