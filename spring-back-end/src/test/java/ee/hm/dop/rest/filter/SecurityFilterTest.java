...
public class SecurityFilterTest {
    ...
    public void filterSessionTimeout() throws IOException, URISyntaxException {
        ...
        expect(authenticatedUser.getSessionTime()).andReturn(java.time.LocalDateTime.now().minusDays(1).minusSeconds(1));
    ...
    private void setExpects(String token, AuthenticatedUser authenticatedUser, User user, String returnedUser, boolean success) {
        ...
        expect(authenticatedUser.getUser()).andReturn(user);
        //todo
        //expect(authenticatedUser.getLoginDate()).andReturn(java.time.LocalDateTime.now().toDateTime(DateTimeZone.UTC)).times(0, 1);
        //expect(authenticatedUser.getSessionTime()).andReturn(java.time.LocalDateTime.now().plusMinutes(15).toDateTime(DateTimeZone.UTC)).times(0, 1);
