...
public class LoginResource extends BaseResource {
    ...
    private URI redirectSuccess(UserStatus status) throws URISyntaxException {
        ...
        return new URI(format(LOGIN_REDIRECT_WITH_TOKEN_AGREEMENT, getServerAddress(), status.getToken(), status.getUserTermsAgreement().toString(), status.getUserTermsAgreement(), status.getGdprTermsAgreement(), status.isExistingUser(), status.getLoginFrom().name()));
    }
