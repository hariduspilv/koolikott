...
public class LoginResource extends BaseResource {
    ...
    private URI redirectSuccess(UserStatus status) throws URISyntaxException {
        ...
        return new URI(format(LOGIN_REDIRECT_WITH_TOKEN_AGREEMENT, getServerAddress(), status.getToken(), status.getUserTermsAgreement(), status.getGdprTermsAgreement(), status.getUserTermsAgreement().toString(), status.isExistingUser(), status.getLoginFrom().name()));
    }
