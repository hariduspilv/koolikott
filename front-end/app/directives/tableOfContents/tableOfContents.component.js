...
class controller extends Controller {
    $onInit() {
        ...
        this.$scope.gotoChapter = (slug, title, evt) => {
            ...
            else if (this.$location.url().includes('&chapterName')) {
                history.pushState({}, '', this.$location.url().split('&chapterName')[0] + '&chapterName=' + titleForUrl + '#' + slug)
            }
            else if (!this.$location.url().includes('&chapterName') && this.$location.hash()) {
                history.pushState({}, '', this.$location.url().split('#chapter')[0] + '&chapterName=' + titleForUrl + '#' + slug)
            }
